...
If $X$ is normally distributed with parameters $\mu$ and $\sigma^2$, then $Z=(X-\mu)/\sigma$ is normally distributed with parameters $\mu=0,\sigma = 1$.
...
an unbiased estimate of the latter is:
...
\begin{enumerate}
\item We compute Maximum likelihood estimates of the mean $\hat\mu$ and standard deviation $\hat\sigma$ to get estimates of the true but unknown parameters.
